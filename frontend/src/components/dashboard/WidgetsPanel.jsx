...
export const WidgetsPanel = () => {
    ...
    const [goalForm, setGoalForm] = useState({ id: null, title: '', targetAmount: '', currentAmount: '' });
    ...
    const handleAddGoal = (e) => {
        e.preventDefault();
        if (goalForm.id) {
            setGoals(goals.map(g => g.id === goalForm.id ? { ...goalForm, icon: g.icon } : g));
        } else {
            setGoals([...goals, { ...goalForm, id: Date.now(), icon: 'Target' }]);
        }
        setShowGoalModal(false);
        setGoalForm({ id: null, title: '', targetAmount: '', currentAmount: '' });
    };
                            ...
                            <div key={goal.id} className="bg-slate-50 dark:bg-[#0B1120] p-4 rounded-2xl border border-slate-200/60 dark:border-white/5">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        ...
                                        <button onClick={() => handleEditGoal(goal)} title="Edit Goal" className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded transition-colors text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><Edit2 className="w-3.5 h-3.5" /></button>
                                        <button onClick={() => handleDeleteGoal(goal.id)} title="Delete Goal" className="p-1 hover:bg-rose-100 dark:hover:bg-rose-500/20 rounded transition-colors text-slate-400 hover:text-rose-500"><Trash2 className="w-3.5 h-3.5" /></button>
                                    </div>
                                </div>
            ...
            <AnimatePresence>
                ...
                {showGoalModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                        ...
                        >
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-bold text-slate-800 dark:text-white">{goalForm.id ? "Edit Goal" : "Add New Goal"}</h3>
                            ...
                            <form onSubmit={handleAddGoal} className="space-y-4">
                                ...
                                <button type="submit" className="w-full mt-2 bg-emerald-600 text-white font-semibold py-3 rounded-xl hover:bg-emerald-700 transition-colors">{goalForm.id ? "Update Goal" : "Save Goal"}</button>
                            </form>
                        </motion.div>
                    </div>
                )}
